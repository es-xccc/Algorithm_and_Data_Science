...
for k in k_values:
    ...
    plt.figure(figsize=(10, 6))
    plt.hist(absolute_errors, bins=range(int(min(absolute_errors)), int(max(absolute_errors)) + 1), alpha=0.7, edgecolor='black')
...
for k in k_values:
    ...
    model.fit(features, targets)
    predictions = model.predict(features)
    percentage_errors = (predictions - targets) / targets * 100
...
print(f'The best k value that gives the maximum R² value is {best_k} with R² = {max_r2:.4f}')