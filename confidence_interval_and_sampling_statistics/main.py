...
plt.figure()
x = np.linspace(min(scores), max(scores), 100)
y = norm.pdf(x, sample_means[0], sample_std_devs[0])
plt.plot(x, y, label='Mean={:.2f}, SD={:.2f}'.format(sample_means[0], sample_std_devs[0]))
plt.title('Normal Distribution for 50 sample size')
y_mean = norm.pdf(sample_means[0], sample_means[0], sample_std_devs[0])
plt.plot([sample_means[0], sample_means[0]], [0, y_mean], color='black')
y_std_dev1 = norm.pdf(mean - 1.96*std_dev, mean, std_dev)
y_std_dev2 = norm.pdf(mean + 1.96*std_dev, mean, std_dev)
plt.plot([mean - 1.96*std_dev, mean - 1.96*std_dev], [0, y_std_dev1], color='red', linestyle='dashed')
plt.plot([mean + 1.96*std_dev, mean + 1.96*std_dev], [0, y_std_dev2], color='red', linestyle='dashed')
plt.legend()
